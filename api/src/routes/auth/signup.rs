...
use crate::infrastructure::{
    common::{ hashing, validator::{ validate_email, validate_password, validate_username } },
...
async fn signup(
        ...
        Ok(user) =>
            match user {
                Some(_) => {
                    return Err(
                        JsonResponse::bad_request(
                            DynamicData::String("Username/Email is already in use".to_string())
                        )
                    );
                }
                None => {}
    ...
    let user_am = match
        database.transaction::<_, User::Model, DbErr>(|txn| {
            Box::pin(async move {
